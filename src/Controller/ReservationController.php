...
$progressBar->finish();
    ...
    {   
        ...
        $roomRequestedId=$reservation->getFkRoomReservation();

        $findStateByRoomId=$CRepository->findOneBy(array('dateroom' => $reservation->getDeparturedate(),
        'fkcalenderroom'=>$roomRequestedId));

        $findStateByRoomId1=$CRepository->findOneBy(array('dateroom' => $reservation->getArrivaldate(),
        'fkcalenderroom'=>$roomRequestedId));
        ...
        if ($form->isSubmitted() && $form->isValid() && $AvailableRoomOnDD!=NULL && $AvailableRoomOnAD!=NULL ) {
            ...
            $findStateByRoomId->setState(1);
            $findStateByRoomId1->setState(1);
     ...
     */
    public function newfront(Request $request, ReservationRepository $reservationRepository, CalenderroomRepository $CRepository,QrcodeService $qrcodeService,ProgressroomRepository $progressroomRepository  ): Response
    {    $qrCode=null;
        $reservation = new Reservation();
        $form = $this->createForm(ReservationType::class, $reservation);
        $form->handleRequest($request);
